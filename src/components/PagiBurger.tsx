...
const PagiBurger = () => {
    ...
    useEffect(() => {
        const fetchBurgers = async () => {
            ...
            setBurgers(response.documents as unknown as MenuItem[]);
            setTotalPages(Math.ceil(response.total / burgersPerPage));
        };
        ...
        <main className='px-4 md:px-0 py-4 md:py-0'>
            ...
            <section className='max-w-6xl mx-auto'>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mx-auto">
                    {burgers.map((burger) => (
                        <div key={burger.title} className="border-2 p-4 rounded-md border-green-600 text-center">
                            <div className="bg-zinc-800 px-2 py-1 rounded-full mb-2 max-w-[60px] mx-auto">
                                <span className="text-sm font-light">{burger.foodnumber}</span>
                            </div>
