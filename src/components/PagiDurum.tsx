...
const PagiDurum = () => {
    const [durums, setDurums] = useState<MenuItem[]>([]);
    const [currentPage, setCurrentPage] = useState(1);
    const [totalPages, setTotalPages] = useState(0);
    const durumsPerPage = 8;

    useEffect(() => {
        const fetchDurums = async () => {
            const response = await databases.listDocuments( process.env.NEXT_APPWRITE_DATABASE_ID, process.env.NEXT_APPWRITE_COLLECTION_DURUM_ID, 
            [
                Query.orderDesc('$createdAt'),
                Query.limit (durumsPerPage),
                Query.offset ((currentPage - 1) * durumsPerPage),
            ]);
            setDurums(response.documents as unknown as MenuItem[]);
            setTotalPages(Math.ceil(response.total / durumsPerPage));
        };
        ...
        <main className='px-4 md:px-0 py-4 md:py-0'>
            ...
            <section className='max-w-6xl mx-auto'>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mx-auto">
                    {durums.map((durum) => (
                        <div key={durum.title} className="border-2 p-4 rounded-md border-green-600 text-center">
                            <div className="bg-zinc-800 px-2 py-1 rounded-full mb-2 max-w-[60px] mx-auto">
                                <span className="text-sm font-light">{durum.foodnumber}</span>
                            </div>
                            <h2 className="font-bold text-xl text-black dark:text-zinc-300">{durum.title}</h2>
                            <p className='py-1 text-black dark:text-zinc-400'>{durum.description}</p>
                            ...
                            <div className='flex justify-center items-center mt-2 gap-3'>
                                <p className="text-zinc-600 dark:text-zinc-400 font-bold mb-2 mt-2">
                                    {durum.price.toFixed(2)}kr
                                </p>
                                {durum.pricediscount > 0 && (
                                    <p className="text-black dark:text-zinc-300 font-semi-bold outline outline-offset-1 outline-red-600 rounded-full px-2 py-0.5">
                                        Tilbud: {durum.pricediscount.toFixed(2)}kr
                                    </p>
                                )}
                            </div>
